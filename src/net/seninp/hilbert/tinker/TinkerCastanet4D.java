...
public class TinkerCastanet4D {

  // http://maps.googleapis.com/maps/api/staticmap?center=43.4485871277,1.5991950798&zoom=11&size=500x500&sensor=false
  ...
  private static final String OUTPUT_PREFIX = "data/castanet/hrm_4D";
  ...
  public static void main(String[] args) throws IOException {
    ...
    ArrayList<double[]> runningPath = getRunningPath();
    ...
    //
    // Point2D.Double staticMapCenterLatLng = new Point2D.Double(43.4385871277, 1.6151950798);
    // http://maps.googleapis.com/maps/api/staticmap?center=43.525071277,1.4991950798&zoom=14&size=500x500&sensor=true
    ...
    //
    Point2D staticMapCentercenter = fromLatLngToPoint2D(staticMapCenterLatLng);
    ...
    CompactHilbertCurve chc = new CompactHilbertCurve(new int[] { HILBERT_CURVE_LEVEL,
        HILBERT_CURVE_LEVEL, HILBERT_CURVE_LEVEL, HILBERT_CURVE_LEVEL });

    // get the graphics
    //
    Graphics g = staticMap.getGraphics();
    g.setColor(GPX_PATH_COLOR);

    for (double[] p : runningPath) {
      // current LatLNg point to X,Y coordinates of 256px MERCATOR map
      double[] point = fromLatLngToPoint4D(p);
      // adjust X and Y according to the current ZOOM level
      double x = (point[0] - leftTop.getX()) * Math.pow(2, ZOOM_LEVEL);
      double y = (point[1] - leftTop.getY()) * Math.pow(2, ZOOM_LEVEL);
      ...
      long zz = (long) Math.floor(point[2]
          / (HILBERT_CURVE_SIZE / Math.pow(2., HILBERT_CURVE_LEVEL)));
      BitVector vector3 = BitVectorFactories.OPTIMAL.apply(HILBERT_CURVE_LEVEL);
      vector3.copyFrom(zz);
      // convert Z to the hilbert cell index
      long hh = (long) Math.floor(point[3]
          / (HILBERT_CURVE_SIZE / Math.pow(2., HILBERT_CURVE_LEVEL)));
      BitVector vector4 = BitVectorFactories.OPTIMAL.apply(HILBERT_CURVE_LEVEL);
      vector4.copyFrom(hh);
      ...
      BitVector[] point3D = { vector1, vector2, vector3, vector4 };
      chc.index(point3D, 0, chi);
      // save that
    ...
    g.setColor(HILBERT_CURVE_COLOR);
    ((Graphics2D) g).draw(hilbertPath);
    g.dispose();

    ImageIO.write(staticMap, "png", new File(OUTPUT_PREFIX + "_map_and_hilbert.png"));

    // create a small chart below
    //
  ...
  private static ArrayList<double[]> getRunningPath() throws NumberFormatException, IOException {
    ...
    while ((line = br.readLine()) != null) {
      ...
      double[] pt = new double[] { Double.valueOf(split[1]), Double.valueOf(split[2]),
          Double.valueOf(split[4]), Double.valueOf(split[8]) };
      points.add(pt);
    }
    br.close();
    ...
    for (double[] p : points) {
      double z = (p[2] - min) / (max - min) * (HILBERT_CURVE_SIZE - 1);
      p[2] = z;
    }
    ...
    for (double[] p : points) {
      double z = (p[3] - min) / (max - min) * (HILBERT_CURVE_SIZE - 1);
      p[3] = z;
    }
   ...
   */
  private static double[] fromLatLngToPoint4D(double[] latlng) {

    // X coordinate
    double x = ORIGIN_X + latlng[1] * PIXELS_PER_LON_DEGREE;

    // Y coordinate
    double siny = Math.sin(degreesToRadians(latlng[0]));
    double y = ORIGIN_Y + 0.5 * Math.log((1 + siny) / (1 - siny)) * (-PIXELS_PER_LON_RADIAN);

    // result
    double[] res = new double[] { x, y, latlng[2], latlng[3] };
    return res;
  };
  ...
  private Point2D fromPointToLatLng(Point2D point) {
   ...
   * @return XY point ready to be set.
   */
  private static Point2D fromLatLngToPoint2D(Point2D latlng) {
