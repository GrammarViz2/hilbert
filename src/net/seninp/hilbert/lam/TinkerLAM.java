...
public class TinkerLAM {

  // define default zoom level
  private final static double ZOOM_LEVEL = 13.;

  private static final int HILBERT_CURVE_LEVEL = 11;
  ...
  private static final String STATIC_MAP_FILENAME = "data/lam/lam-13_staticmap.png";

  private static final String INPUT_PREFIX = "data/lam/processed";
  private static final String OUTPUT_PREFIX = "data/lam/sequences/";
  ...
  public static void main(String[] args) throws IOException {
        ...
        // get all the files
    try (DirectoryStream<Path> dirStream = Files.newDirectoryStream(Paths.get(INPUT_PREFIX),
        "*.csv")) {

      for (Path filePath : dirStream) {

        Path filename = filePath.getFileName();
        String baseName = filename.toString().replace(".csv", "");

        System.out.println("Processing " + filename + ", basename \'" + baseName + "\'");

        BufferedImage staticMap = getStaticMap();
        
        ArrayList<Point2D.Double> runningPath = getRunningPath(filePath.toString());

        // center point of the generated map
        //
        Point2D.Double staticMapCenterLatLng = new Point2D.Double(35.881878, -106.290432);
        ...
        Point2D leftTop = new Point2D.Double(
            staticMapCentercenter.getX() - (HILBERT_CURVE_SIZE / 2.) / Math.pow(2, ZOOM_LEVEL),
            staticMapCentercenter.getY() - (HILBERT_CURVE_SIZE / 2.) / Math.pow(2, ZOOM_LEVEL));

        // hilbert TS construction
        //
        hilbertTS = new ArrayList<Double>();
        CompactHilbertCurve chc = new CompactHilbertCurve(
            new int[] { HILBERT_CURVE_LEVEL, HILBERT_CURVE_LEVEL });
        ...
        ImageIO.write(staticMap, "png", new File(OUTPUT_PREFIX + baseName + "_map.png"));
        ...
        ImageIO.write(staticMap, "png",
            new File(OUTPUT_PREFIX + baseName + "_map_and_hilbert.png"));
        ...
        //
        XYPlot timeseriesPlot = new XYPlot(chartXYSeriesCollection, timeAxis, valueAxis,
            xyRenderer);
        // finally, create the chart
        JFreeChart chart = new JFreeChart("Hilbert curve for " + OUTPUT_PREFIX,
            JFreeChart.DEFAULT_TITLE_FONT, timeseriesPlot, false);
        ChartUtilities.saveChartAsPNG(new File(OUTPUT_PREFIX + baseName + "_hilbert_curve.png"),
            chart, 1280, 250);

        BufferedWriter bw = new BufferedWriter(
            new FileWriter(new File(OUTPUT_PREFIX + baseName + "_hilbert_curve.csv")));
  ...
  private static ArrayList<Point2D.Double> getRunningPath(String fName)
