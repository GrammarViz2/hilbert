package net.seninp.hilbert.anomaly;
...
public class CastanetPlotter {
  ...
  // cornflowerblue
  private static final Color GPX_PATH_COLOR = Color.BLUE;
  ...
  private static final int[] anomaly0 = { 11509, 11878 };
  private static final Color ANOMALY0_COLOR = Color.RED;

  private static final int[] anomaly1 = { 4659, 5025 };
  private static final Color ANOMALY1_COLOR = Color.GREEN;

  private static final int[] anomaly2 = { 6536, 6923 };
  private static final Color ANOMALY2_COLOR = new Color(183, 43, 226);

  private static final int[] anomaly3 = { 14170, 14563 };
  private static final Color ANOMALY3_COLOR = Color.BLUE;
  ...
  public static void main(String[] args) throws IOException {
    ...
    for (int i = 0; i < runningPath.size(); i++) {
      ...
      double x = (point.getX() - leftTop.getX()) * Math.pow(2, ZOOM_LEVEL);
      double y = (point.getY() - leftTop.getY()) * Math.pow(2, ZOOM_LEVEL);
      Shape theCircle = null;
      // draw the anomaly0
      if (anomaly0[0] <= i && i < anomaly0[1]) {
        graphics.setColor(ANOMALY0_COLOR);
        theCircle = new Ellipse2D.Double(x - GPX_POINT_RADIUS, y - GPX_POINT_RADIUS,
            2.0 * GPX_POINT_RADIUS, 2.0 * GPX_POINT_RADIUS);
        ((Graphics2D) graphics).draw(theCircle);
      }

      // draw the anomaly1
      if (anomaly1[0] <= i && i < anomaly1[1]) {
        graphics.setColor(ANOMALY1_COLOR);
        theCircle = new Ellipse2D.Double(x - GPX_POINT_RADIUS, y - GPX_POINT_RADIUS,
            2.0 * GPX_POINT_RADIUS, 2.0 * GPX_POINT_RADIUS);
        ((Graphics2D) graphics).draw(theCircle);
      }
      ...
      // draw the anomaly2
      if (anomaly2[0] <= i && i < anomaly2[1]) {
        graphics.setColor(ANOMALY2_COLOR);
        theCircle = new Ellipse2D.Double(x - GPX_POINT_RADIUS, y - GPX_POINT_RADIUS,
            2.0 * GPX_POINT_RADIUS, 2.0 * GPX_POINT_RADIUS);
        ((Graphics2D) graphics).draw(theCircle);
      }
      ...
      // draw the anomaly3
      if (anomaly3[0] <= i && i < anomaly3[1]) {
        graphics.setColor(ANOMALY3_COLOR);
        theCircle = new Ellipse2D.Double(x - GPX_POINT_RADIUS, y - GPX_POINT_RADIUS,
            2.0 * GPX_POINT_RADIUS, 2.0 * GPX_POINT_RADIUS);
        ((Graphics2D) graphics).draw(theCircle);
      }
  ...
  private static ArrayList<Point2D.Double> getRunningPath() throws NumberFormatException,
    ...
    //
    ArrayList<Point2D.Double> points = new ArrayList<java.awt.geom.Point2D.Double>();
